...
import com.xtool.opencgl.controller.ZookeeperToolController;
    ...
    public void connectOnAction() {
        if (zkClient == null) {
            try {
                ...
                TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "连接失败！！！");
                zkClient = null;
            }
...
//            });
    ...
    public void nodeSelectionChanged(TreeItem<String> selectedItem) {
        ...
        if (!zkClient.exists(nodePath)) {
            TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "节点不存在");
            return;
        }
    ...
    public void refreshOnAction() {
        if (zkClient == null) {
            TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "zookeeper未连接");
            return;
        }
        connectOnAction();
    }
    ...
    public void deleteNodeOnAction() {
        ...
        if (selectedItem == null) {
            TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "未选中节点");
            return;
        }
        String nodePath = this.getNodePath(selectedItem);
    ...
    public void addNodeOnAction() {
        ...
        if (StringUtils.isEmpty(nodeName)) {
            TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "节点名不能为空！");
            return;
        }
        String nodePath = this.getNodePath(selectedItem);
...
//        String nodePath = this.getNodePath(selectedItem);
    ...
    public void addNodeNotify() {//添加节点通知
        ...
        if (childListeners.containsKey(nodePath)) {
            TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "该节点已经添加通知！");
            return;
        }
        IZkChildListener childListener = (parentPath, currentChilds) -> TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "节点Child改变了 " + "Path:" + parentPath + "\r\n 子节点：" + currentChilds.toString());
        zkClient.subscribeChildChanges(nodePath, childListener);
        childListeners.put(nodePath, childListener);
        IZkDataListener dataListener = new IZkDataListener() {
            @Override
            public void handleDataChange(String dataPath, Object data) {
                TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "节点Data改变了 " + "Path:" + dataPath + "\r\n 新数据：" + data.toString());
            }

            @Override
            public void handleDataDeleted(String dataPath) {
                TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "节点删除了 " + "Path:" + dataPath);
            }
        };
        zkClient.subscribeDataChanges(nodePath, dataListener);
        dataListeners.put(nodePath, dataListener);
        TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "该节点添加通知成功！");
    }
    ...
    public void removeNodeNotify() {//移除节点通知
        ...
        TooltipUtil.showToast(zookeeperToolController.getMainAnchorPane(), "该节点通知成功移除！");
    }
}