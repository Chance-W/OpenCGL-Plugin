package com.opencgl.ssh.controller;
...
import com.opencgl.ssh.session.SshSessionTab;
import com.opencgl.ssh.views.SshConfigDialog;
import com.opencgl.ssh.views.SshTerminalView;
 ...
 * 支持同时打开多个SSH连接会话
 */
public class SshTerminalController extends SshTerminalView
    ...
    private void initI18n() {
        ...
        // sshTerminalLabel.textProperty().bind(I18N.getBinding("toolbar.ssh_terminal"));
    }
    ...
    private void handleNodeClick(SshConnectionDto dto) {
        ...
        // 如果当前Tab是未连接的SshSessionTab，直接更新其数据
        if (currentTab instanceof SshSessionTab sessionTab && !sessionTab.isConnected()) {
    ...
    private void disposeSession(Tab tab) {
        if (tab instanceof SshSessionTab sessionTab) {
            try {
                sessionTab.dispose();
            } catch (RuntimeException error) {
                logger.warn("Failed to dispose SSH session tab {}", tab.getText(), error);
            }
        }
    }
    ...
    private void openTabForConnection(SshConnectionDto dto, boolean autoConnect) {
        SshSessionTab newTab;
        if (dto.getHost() != null && dto.getHost().startsWith("LOCAL:")) {
            com.opencgl.ssh.service.LocalShellServiceImpl localService = new com.opencgl.ssh.service.LocalShellServiceImpl();
            newTab = new SshSessionTab("💻 " + dto.getName(), localService);
            newTab.fillConnectionFields(dto);
        } else {
            newTab = new SshSessionTab(dto);
        }
    ...
    public void dispose() {
        lifecycle.dispose(error -> logger.warn("Failed to dispose SSH Terminal controller resource", error),
                () -> {
