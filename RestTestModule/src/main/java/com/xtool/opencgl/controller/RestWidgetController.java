...
import com.xtool.opencgl.dao.RestWidgetDao;
...
import com.xtool.opencgl.util.EditableTextFieldTreeTableCell;
import com.xtool.opencgl.views.RestWidgetView;
...
public class RestWidgetController extends RestWidgetView implements Initializable, TreeOperateService<RestWidgetDto> {
    ...
    public void sendLabelAction() {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            String requestText;
            if (inputTextArea.getText().contains("access_token")
                && inputTextArea.getText().contains("method")
                && inputTextArea.getText().contains("version")
                && inputTextArea.getText().contains("content")) {
                JSONObject jsonObject = JSONObject.parseObject(inputTextArea.getText());
                jsonObject.put("content", FormatVariableUtil.format(jsonObject.get("content").toString()));
                requestText = JSONObject.toJSONString(jsonObject);
            }
            else {
                requestText = FormatVariableUtil.format(inputTextArea.getText());
            }
