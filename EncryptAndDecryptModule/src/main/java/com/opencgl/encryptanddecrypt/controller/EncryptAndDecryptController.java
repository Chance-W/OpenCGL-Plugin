package com.opencgl.encryptanddecrypt.controller;
...
import com.opencgl.encryptanddecrypt.views.EncryptAndDecryptView;
import com.opencgl.encryptanddecrypt.utils.GuiUtils;
import com.opencgl.encryptanddecrypt.utils.JavaFxViewUtil;
import com.opencgl.encryptanddecrypt.utils.MorseConventer;
...
public class EncryptAndDecryptController extends EncryptAndDecryptView {
    ...
    private void initI18n() {
        ...
        charsetLabel.textProperty().bind(I18N.getBinding("label.charset"));
        keyLabel.textProperty().bind(I18N.getBinding("label.key"));
        encrptyButton.textProperty().bind(I18N.getBinding("button.encrypt"));
        decrptyButton.textProperty().bind(I18N.getBinding("button.decrypt"));
    }
    ...
    private void initView() {
        ...
        for (int i = 0; i < cryptos.length; i++) {
                if (!"".equals(cryptos[i])) {
                String displayText = "文件加密MD5".equals(cryptos[i]) ? I18N.get("crypto.file_md5")
                    : "文件加密SHA1".equals(cryptos[i]) ? I18N.get("crypto.file_sha1")
                    : "摩斯密码".equals(cryptos[i]) ? I18N.get("crypto.morse")
                    : "Druid加密".equals(cryptos[i]) ? I18N.get("crypto.druid")
                    : cryptos[i];
                MFXRadioButton radioButton = new MFXRadioButton(displayText);
                radioButton.setLayoutX(440 + i % 3 * 150);
                radioButton.setLayoutY(60 + (double) i / 3 * 26);
                radioButton.setToggleGroup(toggleGroup);
                radioButton.setUserData(cryptos[i]);
                encryptRootPane.getChildren().add(radioButton);
                if (i == 0) {
    ...
    private void encrptyAction(ActionEvent event) {// 加密
        ...
        try {
            if (GuiUtils.CRYPTO_ASCII.equals(curCrypto)) {
                ...
                decrptyTextArea.setText(I18N.get("msg.16bit") + md5Val.substring(8, 24) + "\n" + I18N.get("msg.32bit") + md5Val);
            }
...
//                encrptyTextArea.setText(new String(decryptBytes));
                ...
                encrptyTextArea.setText(I18N.get("msg.decrypt_not_supported"));
            }
        }
        catch (Exception e) {
