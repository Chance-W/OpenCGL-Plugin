...
//				user.setEnabled(ftpServerTableBean.getIsEnabled());
    ...
    public boolean stopFtpServerAction() {
        FtpServer serverToStop = server;
        server = null;
        if (serverToStop != null) {
            try {
                serverToStop.stop();
            } catch (RuntimeException e) {
                log.error("停止 FTP Server 失败", e);
            }
        }
        return true;
    }
    ...
    public void saveConfigure(File file) throws Exception {
        ...
        xmlConfigure.save();
        TooltipUtil.showToast(ftpServerController.getMainAnchorPane(), "保存配置成功,保存在：" + file.getPath());
    }
    ...
    public void loadingConfigure(File file) {
        try {
            ...
            PropertiesConfiguration xmlConfigure = new PropertiesConfiguration(file);
            xmlConfigure.getKeys().forEachRemaining(t -> ftpServerController.getTableData().add(new FtpServerTableBean(xmlConfigure.getString(t))));
        } catch (Exception e) {
            try {
                log.error("加载配置失败：" + e.getMessage());
                TooltipUtil.showToast(ftpServerController.getMainAnchorPane(), "加载配置失败：" + e.getMessage());
