package com.opencgl.brower.utils;
...
@Setter
public
class WebSourcesToolService {

    private static final Logger log = LoggerFactory.getLogger(WebSourcesToolService.class);
	...
	public void downloadHtmlSources() throws Exception {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			indexHtml = Objects.requireNonNull(response.body()).string();
		}
		if (new File(sourcesFilePathString).mkdirs()){
			log.info("文件创建成功");
		}else {
			log.info("文件创建失败");
		}
	...
	public void dispose() {
		webSourcesToolController = null;
		client.dispatcher().cancelAll();
		client.dispatcher().executorService().shutdownNow();
		client.connectionPool().evictAll();
		if (client.cache() != null) {
			try { client.cache().close(); } catch (IOException error) { log.warn("Failed to close HTTP cache", error); }
		}
	}
